...
impl Default for TransitionTimer {
    fn default() -> Self {
        Self {
            ...
            timeouts: vec![Timer::new(Duration::from_millis(2000), TimerMode::Once)],
            current_index: 0,
...
impl TransitionTimer {
    pub(crate) fn update(&mut self, time: Time) -> Option<WoodpeckerStyle> {
        if self.timeouts.iter().all(|timer| timer.finished()) {
            return None;
        }
        // if we haven't started, tick start timer
        if !(self.start.finished() || self.start.tick(time.delta()).just_finished()) {
            return Some(self.styles[0]);
        };

        let Some(next_timer) = self.timeouts.get_mut(self.current_index) else {
        ...
        Some(self.styles[self.current_index].lerp(&self.styles[self.current_index + 1], x))
    }
...
pub fn update_transitions(
    mut query: Query<(&mut TransitionTimer, &mut WoodpeckerStyle)>,
    time: Res<Time>,
) {
    for (mut transition, mut styles) in query.iter_mut() {
        let Some(new_styles) = transition.update(*time) else {
